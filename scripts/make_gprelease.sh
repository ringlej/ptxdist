...
case "${branch}" in
    production)
	release="${v}$(date +%Y.%m).0_GP"
	prev_release="${v}*.0_GP"
	;;
    release/*)
	release="${branch##release/}"
	release="${release%.x_GP}"
	inc="$(git tag -l "${release}.*" | wc -l)"
	if [ ${inc} -eq 0 ]; then
	    echo "about to make stable a release for '${release}', but no '.0' found" >&2
	    exit 1
	fi
	prev_release="${release}.$((inc - 1))_GP"
	release="${release}.${inc}_GP"
	;;
    *)
	echo "please checkout either production or release branch" >&2
	exit 1
	;;
esac
